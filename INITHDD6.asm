...
L_05CY:	CALL    L_0B3F	; вывод числа из HL
	...
	LHLD    L_0F0B
	XCHG
	LXI  H, 029BEh	; максимальное количество дискет
	MOV  A, D
	ORA  A
	JNZ	L_PDT	; > 00FF-FFFFh
	LHLD    L_0F09	; EHL -- всего секторов LBA
	LXI  B, 0FFAEh	; -52h секторов на загрузочную область
	DAD  B
	MOV  A, E	; старшие 8 бит
	ACI     0FFh	; число секторов AHL -= 52h
	LXI  D, 00000h	; результат
	JNC	L_CDO	; >>>
...
;
L_CDO:	XCHG	
L_PDT:	SHLD    D_DSKT	; максимально возможное число дискет +1
	CALL    L_0A56	; вывод cтроки до "$" с добавлением перевода строки в начале
